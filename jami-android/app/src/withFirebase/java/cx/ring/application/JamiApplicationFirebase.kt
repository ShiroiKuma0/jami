...
/**
 * Firebase-only flavor. The background account-deactivation machinery this class used to own
 * (grace windows, episode cap, process lifecycle observer) moved to [JamiApplication] on
 * 2026-07-29 so the shipped dual-backend flavor gets it too; only the FCM-specific token
 * handling stays here.
 */
@HiltAndroidApp
class JamiApplicationFirebase : JamiApplication() {
    ...
    fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // A delivered push is the proof the base class requires before it will let the accounts
        // sleep (ConnectionWatchdog.backgroundSleepSafe); this flavor has no other way to earn it.
        cx.ring.utils.PushEvidence.noteRealPush()
