...
class Conversation(
    ...
    @Synchronized
    fun addCall(callHistory: CallHistory) {
    ...
    private val callHistory: Collection<CallHistory>
        get() {
            val result: MutableList<CallHistory> = ArrayList()
            for (interaction in aggregateHistory) {
                if (interaction.type == Interaction.InteractionType.CALL) {
                    result.add(interaction as CallHistory)
                }
            }
            return result
        }
    ...
    fun addSwarmElement(interaction: Interaction, newMessage: Boolean) {
        ...
        if (interaction is CallHistory && interaction.confId != null) {
